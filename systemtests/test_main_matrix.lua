...
function ErrLogOpen()
    if(not Errlog) then
       ...
       Errlog:write("\nHost OS: " .. gma.show.getvar("OS") .. ". GrandMA version: " .. host_version ..". Hostsystem type: " .. gma.show.getvar("HOSTTYPE") .. ". Hostsystem IP: " .. gma.network.getprimaryip())
...
function StartSingleTestScript(test_script)
    ...
    if not testmatrix_started then
        ...
        prop = loadProperties();
        debugMode = check_debug(prop, test_script.method_name, test_script.object_name);
        if dev then
        end
            F("Debug: " .. tostring(debugMode));
    end
    local step_skips = { };
...
local function StartAllTestScripts(oldtime)
    ...
    if tonumber(amount) > 0 then
        ...
        local assertFailed, assertLog = false,'';
        -- make sure these tests run without error:
        for i=1, #AssertFunctions do
            local passed, error = xpcall(AssertFunctions[i],debug.traceback,C,F,E);
            if not passed then
                F('luatest error in AssertFunction: '..error);
                cancel = true;
                assertFailed = true;
                assertLog = error;
            end
                ...
                for object_index,test_script in ipairs(method) do
                    ...
                    if(test_script and type(test_script)=="table") then
                        ...
                        if dev then
                            ...
                            F("Debug Result: " .. tostring(debugMode));
                        end
        ...
        if assertFailed then
            F('Assert Test failed!');
            F(assertLog);
        elseif script_errors > 0 then
...
local function CleanUp()
    ...
    F("CleanUp Done");
    if not server then
        F("Shutdown cancelled (Server Flag is not set).")
    end
    if server == true then
